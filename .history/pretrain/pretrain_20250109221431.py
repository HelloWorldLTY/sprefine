...
# adata = sc.read_h5ad("./spe_xenium_withMetrics.h5ad")
adata = sc.read_h5ad("/home/tl688/pitl688/test_image/visium_all.h5ad")

sc.pp.subsample(adata, random_state=2024, fraction=0.1)
...
meta_df_t_update['disease_update'].value_counts()
meta_df_t_update
...
adata.obs['batch'] = ['hest_' + str(i) for i in adata.obs['batch'].values ]





adata_stimage = sc.read_h5ad("/home/tl688/pitl688/ref_free_imp/adata_stimageprofile.h5ad")

adata_stimage.obs['batch'] = ['stimage_' + str(i) for i in adata_stimage.obs['batch'].values ]

import numpy as np

adata_dense = adata_stimage.X

filled_arr = np.where(np.isnan(adata_dense), 0, adata_dense )

adata_stimage.X = filled_arr.copy()
...
adata.obs = adata.obs[["batch", "species"]]

adata.obsm['spatial']

adata_stimage.obsm['spatial'] = adata_stimage.obsm['spatial'].values[:,0:2]

adata_final = sc.concat([adata_stimage,adata])

adata_final.write_h5ad("/home/tl688/pitl688/ref_free_imp/adata_stimage_hest.h5ad")
